---
name: release
description: Release skill for vivid-life-fish — bumps version, updates CHANGELOG, commits, tags, pushes, and creates a GitHub release. There is no package registry for this port; the GitHub Release is the only release surface. Use only when intentionally cutting a release.
disable-model-invocation: true
---

# Release Skill — vivid-life-fish

This port has no package registry (fish themes are installed by cloning/copying files, not via a package manager). "Release" here means: a tagged commit plus a GitHub Release, so anyone opening the repo on GitHub can see there's a new version without having to `git pull` and diff themselves.
...
- Patch (X.Y.Z+1): bug fixes, documentation updates, palette/color tweaks
- Minor (X.Y+1.0): new flavor, new variant, new theme file, new feature
- Major (X+1.0.0): breaking changes — renamed/removed theme files, changed installation path or naming convention consumers rely on
...
> Tag vX.Y.Z pushed and release published: https://github.com/vivid-life-theme/vivid-life-fish/releases/tag/vX.Y.Z
